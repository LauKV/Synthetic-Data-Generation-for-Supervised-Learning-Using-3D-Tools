...
def _set_camera_at_size(i: int, scale: float = 1.5) -> bpy.types.Object: #--------------------------------- Remove?
...
def load_object(object_path: str) -> None:
    ...
    if file_extension == "usdz": #----------------------------------------------- Remove? Not using this kind of files?
...
def delete_invisible_objects() -> None: # Remove?
...
def delete_missing_textures() -> Dict[str, Any]: # ----------------------Remove?
...
def _get_random_color() -> Tuple[float, float, float, float]:
    ...
    Returns:
        ...
        range [0, 1].
    """
    return (random.random(), random.random(), random.random(), 1) # (1,0.5, 75,1) for pink


def _apply_color_to_object( # ------------------------------------ Remove?
...
def apply_single_random_color_to_all_objects() -> Tuple[float, float, float, float]: # ----------- Remove?
...
def render_object(
    ...
    if object_file.endswith(".blend"):
        ...
        reset_cameras()
        # delete_invisible_objects() #remove?
    else:
        reset_scene()
        load_object(object_file) #,obj_name=object_file, center=True)

    obj = bpy.context.object  # Get the currently selected object
    normalize_obj(obj) # Normalize the object
    ...
    if object_file.lower().endswith(".usdz"): # ------------------- Remove?
    ...
    # normalize_obj(load_object(object_file)) #  <-----------------------------------------
    ...
    for i in range(num_renders):
        ...
        set_colored_background(color=(1, 1, 1, 1))
        # set_hdri_background(hdri_path="/home/opdal/Bureau/5GMM/small_cathedral_02_4k.exr")    ###### HDRI Background path
...
if __name__ == "__main__":
    ...
    # set a collored background 
    #set_colored_background(color= (128,0,0,1)) #(0, 105, 92,1))# color=(1, 0, 0, 1))red 

    # set_hdri_background(hdri_path="/home/opdal/Bureau/5GMM/hilly_terrain_01_4k.exr")
