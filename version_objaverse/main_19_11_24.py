...
def get_example_objects() -> pd.DataFrame:                                                          ### originale fonction
    """Returns a DataFrame of example objects to use for debugging."""
    #return pd.read_json("example-objects.json", orient="records") 
    return pd.read_json("all_adresses_objets.json", orient="records")

def get_n_objects(n: int) -> pd.DataFrame:                                                     ### My fonction
    """Returns a DataFrame of `n` randomly selected example objects for debugging.

    Args:
        n (int): The number of random addresses to retrieve.

    Returns:
        pd.DataFrame: A DataFrame containing up to `n` randomly sampled rows from all_adresses_objets.json.
    """
    # Load the JSON file into a DataFrame
    df = pd.read_json("all_adresses_objets.json", orient="records")                                   ######## all_adresses_objets.json or example-objects.json
    
    # Determine the sample size, allowing sampling with replacement if n > len(df)
    sample_size = min(n, len(df))
    return df.sample(sample_size, replace=(n > len(df)))

import pandas as pd

def get_n_objects_type_obj(n: int) -> pd.DataFrame:                                                     ### My fonction
    """Returns a DataFrame of `n` randomly selected objects of type `.obj`.

    Args:
        n (int): The number of random .obj objects to retrieve.

    Returns:
        pd.DataFrame: A DataFrame containing up to `n` randomly sampled .obj objects.
    """
    # Load the JSON file into a DataFrame
    df = pd.read_json("all_adresses_objets.json", orient="records")
    
    # Filter for objects of type .obj
    obj_df = df[df["fileIdentifier"].str.endswith(".obj")]
    
    # Determine the sample size, allowing sampling with replacement if n > len(obj_df)
    sample_size = min(n, len(obj_df))
    return obj_df.sample(sample_size, replace=(n > len(obj_df)))

def get_n_objects_type_blend(n: int) -> pd.DataFrame:                                                     ### My fonction
    """Returns a DataFrame of `n` randomly selected objects of type `.blend`.
    ...
    # Filter for objects of type .obj
    obj_df = df[df["fileIdentifier"].str.endswith(".blend")]
...
def render_objects(
    ...
    render_dir: str = "render_outputs_obj",                                                   #### Fold name where renders will be
    download_dir: Optional[str] = None,
    num_renders: int = 5,                                                                       ####################################### 12
    ...
    objects = get_n_objects_type_obj(3)                           ### use get_n_objects(n) or get_n_objects_type_obj(n) or get_n_objects_type_blend(n) or get_example_objects()
