...
def main(args):
    ...
    enable_segmentation_render(f"{output_dir}/segmentation", max_value=len(objects))
    enable_depth_render(f"{output_dir}/depth", reverse=True)
    enable_normal_render(f"{output_dir}/normal")
    ...
    for scene_idx in range(args.num_scenes):
        for obj_idx, obj in enumerate(objects):
            ...
            for view_idx in range(args.num_views):
                ...
                base_filename = f"{output_dir}/render_{render_counter:05d}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_scenes", type=int, default=10, help="Number of scenes to render")
